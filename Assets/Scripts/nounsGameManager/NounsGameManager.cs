...
public class NounsGameManager : MonoBehaviour
{
    ...
    // Unified question data structure for all difficulties
    [System.Serializable]
    ...
    private UnifiedQuestionData[] allQuestions = {
        ...
        new UnifiedQuestionData {
            ...
            blankWord = "aso",
            acceptableAnswers = new string[] {"aso", "mga aso", "ang aso"},
            xpReward = 20
        },
        new UnifiedQuestionData {
            questionId = 12,
    ...
    void LoadDifficultyFromSceneController()
    {
        if (SceneController.Instance != null)
        {
            ...
            if (selectedDifficulty != DifficultyLevel.Easy) // Only update if not default
            {
    ...
    void StartGame()
    {
        ...
        // Show choice buttons, hide continue button
        foreach (Button button in choiceButtons)
        {
    ...
    void DisplayChoices(string[] choices)
    {
        ...
        // Don't display choices if we're in summary mode
        if (isInSummaryMode)
        {
            Debug.Log("🔒 DisplayChoices blocked - in summary mode");
            return;
        }
    ...
    {
        // Prevent clicks if in summary mode
        if (isInSummaryMode)
        {
            Debug.Log("🔒 Answer selection blocked - in summary mode");
            return;
        }
    ...
    void ProcessAnswer(string selectedAnswer, string correctAnswer, bool isCorrect, int buttonIndex)
    {
        ...
        if (reviewQuestions.Count == 0 && currentQuestion < questions.Length)
        {
            ...
            Debug.Log($"Answer check: '{selectedAnswer}' vs '{correctAnswer}' = {isCorrect}");
        }
    ...
    void EnsureChoicesHidden()
    {
        // Only run this check if we're in summary mode
        if (!isInSummaryMode)
        {
            CancelInvoke(nameof(EnsureChoicesHidden));
            return;
        }
        
        // Check if any choice buttons are visible and hide them
        if (choiceButtons != null)
        {
            foreach (Button button in choiceButtons)
            {
                ...
                {
                    button.gameObject.SetActive(false);
                    Debug.Log($"🔒 Ensured choice button hidden: {button.name}");
                }
            }
        }
        
        // Also check conversation buttons
        if (conversationButtons != null)
        {
            foreach (Button button in conversationButtons)
            {
                ...
                {
                    button.gameObject.SetActive(false);
                    Debug.Log($"🔒 Ensured conversation button hidden: {button.name}");
                }
            }
        }
    ...
    {
        // Show continue button for session summary completion
        continueButton.gameObject.SetActive(true);
        continueButton.onClick.RemoveAllListeners();
        continueButton.onClick.AddListener(OnSummaryContinue);
        
        // Update button text for summary
        ...
        // Set flag to indicate we're in summary mode
        isInSummaryMode = true;
        
        // Start periodic checking to ensure choice buttons stay hidden
        InvokeRepeating(nameof(EnsureChoicesHidden), 0.5f, 0.5f);
    }
    ...
    void EndGame()
    {
        Debug.Log("🎯 EndGame called - hiding choice buttons and showing summary");
        
        // Set summary mode flag FIRST to prevent any choice display
        isInSummaryMode = true;
    ...
    private void DisplaySessionSummary()
    {
        ...
        float averageResponseTime = questionResponseTimes.Values.Average();
        
        string summary = $"🎉 Session Complete!\n\n";
        summary += $"📊 Session Accuracy: {sessionAccuracy:F1}%\n";
        summary += $"⏱️ Average Response Time: {averageResponseTime:F1}s\n";
        summary += $"🏆 Total Points: {score}\n\n";
        
        // Add SM2 data if available
        if (SM2Algorithm.Instance != null)
        {
            var userProgress = SM2Algorithm.Instance.GetUserProgress();
            float overallAccuracy = SM2Algorithm.Instance.GetOverallAccuracy();
            int moduleScore = SM2Algorithm.Instance.GetModuleScore("Nouns");
            float moduleMastery = SM2Algorithm.Instance.GetModuleMastery("Nouns");
            
            summary += $"📈 Overall Accuracy: {overallAccuracy:F1}%\n";
            summary += $"🎯 Module Score: {moduleScore}\n";
            summary += $"🧠 Module Mastery: {moduleMastery:F1}%\n";
            summary += $"🔥 Current Streak: {userProgress.currentStreak}\n";
            summary += $"⭐ Level: {userProgress.level}\n";
            summary += $"💎 Total XP: {userProgress.experience}\n\n";
        }
        else
        {
            Debug.LogWarning("SM2Algorithm.Instance is null in DisplaySessionSummary");
        }
        ...
        if (LearningAnalytics.Instance != null)
        {
            ...
            if (insights != null)
            {
                summary += $"🎓 Learning Style: {insights.learningStyle}\n";
                summary += $"💡 Tip: {LearningAnalytics.Instance.GetPersonalizedStudyTip()}\n\n";
            }
        }
        ...
        if (GamificationSystem.Instance != null)
        {
            var recentAchievements = GamificationSystem.Instance.GetUnlockedAchievements()
                .Where(a => (DateTime.Now - a.unlockedDate).TotalMinutes < 5) // Recent achievements
                .ToList();
        ...
        if (sessionAccuracy >= 90f)
            summary += "🌟 Excellent! Perfect performance!";
        else if (sessionAccuracy >= 80f)
            summary += "🎉 Great work! Keep it up!";
        else if (sessionAccuracy >= 60f)
            summary += "👍 Good progress! You're learning!";
        else
            summary += "💪 Keep practicing! Don't give up!";
            
        // End session tracking
        if (SM2Algorithm.Instance != null)
        {
            SM2Algorithm.Instance.EndSession("Nouns", sessionTotalAnswers, sessionCorrectAnswers, averageResponseTime);
        }
            
        // Use adaptive dialog system for session summaries
        AdaptiveDialogManager adaptiveDialog = FindObjectOfType<AdaptiveDialogManager>();
        if (adaptiveDialog != null)
        {
            adaptiveDialog.ShowSessionSummary(summary, () => {
                Debug.Log("Session summary completed");
                // Ensure choice buttons are still hidden
                Invoke(nameof(ForceHideChoices), 0.1f);
                // Show continue button after summary is displayed
                ShowSummaryContinueButton();
            });
        }
        else if (typewriterEffect != null)
        {
            // Use typewriter effect for session summaries
            typewriterEffect.StartTypewriter(summary, true);  // true = session summary mode
            typewriterEffect.OnTypingCompleted += () => {
                // Ensure choice buttons are still hidden
                Invoke(nameof(ForceHideChoices), 0.1f);
                // Show continue button after typewriter completes
                ShowSummaryContinueButton();
            };
        }
        else if (dialogText != null)
        {
            ...
            // Ensure choice buttons are hidden and show continue button after delay
            Invoke(nameof(ForceHideChoices), 0.1f);
            Invoke(nameof(ShowSummaryContinueButton), 2f);
        }
        else
        {
            Debug.LogError("Both adaptive dialog and typewriter systems are unavailable for session summary");
        }
    }
    ...
    void ShowEasyUI()
    {
        ...
        if (choiceButtons != null && currentQuestionData.choices != null)
        {
            for (int i = 0; i < choiceButtons.Length && i < currentQuestionData.choices.Length; i++)
            {
                if (choiceButtons[i] != null)
                {
                    ...
                    if (buttonText != null)
                    {
                        buttonText.text = currentQuestionData.choices[i];
                    }
                }
            }
        }
    ...
    void ShowMediumUI()
    {
        ...
        {
            sentenceText.text = currentQuestionData.sentenceTemplate;
        }
        
        if (mediumInstructionText != null)
        {
            mediumInstructionText.text = currentQuestionData.instruction;
        }
        
        if (fillInBlankInput != null)
        {
            fillInBlankInput.text = "";
            fillInBlankInput.ActivateInputField();
        }
    }
    ...
    void OnChoiceButtonClick(int choiceIndex)
    {
        ...
        if (isInSummaryMode)
        {
            Debug.Log("🔒 Choice button click blocked - in summary mode");
            return;
        }
    ...
    void OnContinue()
    {
        // Check if we're in summary mode first
        if (isInSummaryMode)
        {
            OnSummaryContinue();
            return;
